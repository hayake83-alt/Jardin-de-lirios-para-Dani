...
{name:"Lirio araña rojo",type:"spider-red",note:"Recuerdos intensos y emociones profundas.",message:"Incluso tus emociones más complejas tienen una belleza que merece ser comprendida."},
{name:"Lirio araña azul",type:"spider-blue",note:"Introspección, misterio y serenidad.",message:"Tu mundo interior es profundo, extraño y hermoso, como una flor bajo la luz de la luna."},
{name:"Lirio del valle",type:"valley",note:"Ternura, dulzura y pequeños detalles.",message:"Tu ternura vive en cosas pequeñas que, para quien te ama, significan muchísimo."},
...
function build(){
 ...
 for(let i=0;i<(phone?620:1080);i++){
   const d=rnd(),y=hor+Math.pow(d,1.42)*(h-hor-3),pers=.18+d*1.22,type=pool[Math.floor(rnd()*pool.length)];
   let size=rr(6.5,15.5)*pers;if(type==="valley")size*=.78;if(type.startsWith("spider"))size*=.95;
 ...
 for(let i=0;i<TARGET;i++){
   const dx=w*(.11+i*(.78/(TARGET-1))),dd=.57+(i%3)*.1,dt=meta[i].type;let best=null,score=1e9;
...
function petal(ctx,L,W,A,C,V){
 ctx.save();ctx.rotate(A);const q=ctx.createLinearGradient(0,2,0,-L);q.addColorStop(0,C[0]);q.addColorStop(.48,C[1]);q.addColorStop(1,C[2]);ctx.fillStyle=q;ctx.strokeStyle="rgba(255,238,228,.26)";ctx.lineWidth=Math.max(.42,W*.034);
 ctx.beginPath();ctx.moveTo(0,2);ctx.bezierCurveTo(-W*.76,-L*.22,-W*.72,-L*.68,0,-L);ctx.bezierCurveTo(W*.72,-L*.68,W*.76,-L*.22,0,2);ctx.closePath();ctx.fill();ctx.stroke();
 ctx.strokeStyle=V;ctx.lineWidth=Math.max(.32,W*.022);ctx.beginPath();ctx.moveTo(0,0);ctx.quadraticCurveTo(W*.06,-L*.45,0,-L*.85);ctx.stroke();
 for(let i=0;i<5;i++){ctx.fillStyle="rgba(95,23,40,.32)";ctx.beginPath();ctx.arc((i-2)*W*.1,-L*(.42+i*.07),Math.max(.28,W*.02),0,6.28);ctx.fill()}ctx.restore();
}
function pal(t,i){
...
function classic(ctx,f,x,y,z){
 ctx.save();ctx.translate(x,y);ctx.rotate(f.rot);for(let i=0;i<6;i++){const p=pal(f.type,i);petal(ctx,z*(1.08+(i%2)*.07),z*.52,i*Math.PI/3,p.slice(0,3),p[3])}
 ctx.fillStyle="#f0d69b";ctx.beginPath();ctx.arc(0,0,z*.19,0,6.28);ctx.fill();
 for(let i=0;i<6;i++){const a=i*Math.PI/3+.22,L=z*(.54+(i%2)*.08);ctx.strokeStyle="rgba(244,214,151,.95)";ctx.lineWidth=Math.max(.5,z*.04);ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(Math.cos(a)*L,Math.sin(a)*L);ctx.stroke();ctx.fillStyle="#4b1c17";ctx.beginPath();ctx.ellipse(Math.cos(a)*L,Math.sin(a)*L,z*.05,z*.095,a,0,6.28);ctx.fill()}ctx.restore();
}
...
function flower(ctx,f,x,y,o={}){
 ...
 if(f.type==="spider-red")spider(ctx,f,hx,hy,z,false);else if(f.type==="spider-blue")spider(ctx,f,hx,hy,z,true);else if(f.type==="valley")valley(ctx,f,hx,hy,z);else classic(ctx,f,hx,hy,z);
...
function bouquetLayout(){const phone=s.bw<800,cx=phone?s.bw*.46:s.bw*.47,by=s.bh*.83,sc=Math.max(.58,Math.min(1.16,Math.min(s.bw*.55,s.bh*.78)/430));return{cx,by,sc,p:[[-112,14,.88,-.22],[-76,-38,.94,-.14],[-38,8,.92,-.07],[0,-72,1.06,0],[38,8,.92,.07],[76,-38,.94,.14],[112,14,.88,.22]]}}
...
function wrap(L){b.save();b.translate(L.cx,L.by);let q=b.createLinearGradient(-70*L.sc,-10,70*L.sc,125*L.sc);q.addColorStop(0,"rgba(239,210,170,.97)");q.addColorStop(.48,"rgba(184,145,104,.94)");q.addColorStop(1,"rgba(103,72,55,.96)");b.fillStyle=q;b.beginPath();b.moveTo(-66*L.sc,-20*L.sc);b.lineTo(66*L.sc,-20*L.sc);b.lineTo(34*L.sc,124*L.sc);b.lineTo(-34*L.sc,124*L.sc);b.closePath();b.fill();b.fillStyle="#77182f";b.beginPath();b.ellipse(0,18*L.sc,58*L.sc,14*L.sc,0,0,6.28);b.fill();b.fillStyle="#a6334d";for(const side of [-1,1]){b.beginPath();b.moveTo(side*5*L.sc,16*L.sc);b.bezierCurveTo(side*46*L.sc,-10*L.sc,side*65*L.sc,28*L.sc,side*18*L.sc,42*L.sc);b.bezierCurveTo(side*42*L.sc,64*L.sc,side*4*L.sc,68*L.sc,-side*8*L.sc,29*L.sc);b.fill()}b.restore()}
function renderBouquet(dt){bouquetBg();const L=bouquetLayout();s.hits=[];for(let i=0;i<s.selected.length;i++){const q=s.selected[i],p=L.p[i],f={...q,size:28*p[2]*L.sc,stem:178*L.sc,rot:p[3],selected:false,special:false,d:1},x=L.cx+p[0]*L.sc,y=L.by+p[1]*L.sc,h=flower(b,f,x,y,{bouquet:true});s.hits.push({x:h.x,y:h.y,r:Math.max(22,h.r),mi:q.mi})}wrap(L);particles(b,s.bpollen,dt)}
