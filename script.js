(() => {
  ...
  function buildField() {
    ...
    grass.length = 0;
    for (let i = 0; i < Math.max(120, Math.floor(state.width / 3)); i++) {
      grass.push({ x: Math.random() * state.width, y: rand(state.height * 0.68, state.height), h: rand(15, 72), lean: rand(-0.8, 0.8), phase: rand(0, Math.PI * 2), alpha: rand(0.08, 0.32) });
    }
  ...
  function background() {
    ...
    ctx.beginPath(); ctx.moveTo(0, state.height * 0.75); ctx.quadraticCurveTo(state.width * 0.5, state.height * 0.69, state.width, state.height * 0.77); ctx.lineTo(state.width, state.height); ctx.lineTo(0, state.height); ctx.closePath(); ctx.fill();
  }
  ...
  function grassLayer() {
    ...
    grass.forEach(b => {
      const wind = reducedMotion ? 0 : Math.sin(state.time * 0.0011 + b.phase) * 7;
      ctx.strokeStyle = `rgba(72,101,58,${b.alpha})`; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(b.x, b.y); ctx.quadraticCurveTo(b.x + b.lean * b.h * 0.25 + wind * 0.35, b.y - b.h * 0.5, b.x + b.lean * b.h + wind, b.y - b.h); ctx.stroke();
    });
    ctx.restore();
  }
  ...
  function lilyHead(f, x, y, sway, scaleMul = 1, alpha = 1, bouquetIndex = -1) {
    ...
    if (f.selectable && !f.selected && !state.completed) {
      ...
      const halo = ctx.createRadialGradient(0, -4 * s, 0, 0, -4 * s, 48 * s);
      halo.addColorStop(0, f.colorType === "white" ? `rgba(255,247,226,${0.18 + pulse * 0.12})` : `rgba(255,99,113,${0.18 + pulse * 0.12})`); halo.addColorStop(1, "rgba(255,255,255,0)");
      ctx.fillStyle = halo; ctx.beginPath(); ctx.arc(0, -4 * s, 50 * s, 0, Math.PI * 2); ctx.fill(); ctx.restore();
    }
  ...
  function findFieldFlower(x, y) {
    let best = null, bestD = Infinity;
    flowers.forEach(f => {
      if (!f.selectable || f.selected || state.completed) return;
      const p = headPos(f), d = Math.hypot(x - p.x, y - p.y), r = Math.max(28, 43 * f.scale);
  ...
  function chooseFlower(f) {
    ...
    const p = headPos(f); burst(p.x, p.y, f.colorType === "white" ? "#fff0d7" : "#ff7181", 20);
  ...
  function tap(event) {
    ...
    if (state.completed) { const area = findBouquetFlower(x, y); if (area) openFlowerMessage(area); }
    else chooseFlower(findFieldFlower(x, y));
  }
